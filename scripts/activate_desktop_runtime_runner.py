#!/usr/bin/env python3
"""Bootstrap governado do runner isolado do Desktop PC24x7 Runtime.

- host fixo DESKTOP-PDQK954;
- repositório, nome, labels e diretório fixos;
- runner oficial com versão e SHA-256 fixados;
- token de registro efêmero apenas em memória;
- nunca reutiliza, remove ou reconfigura o runner legado do ReqSys;
- reutiliza apenas as funções de identidade/processo do watchdog versionado,
  sem instalar ou substituir a tarefa watchdog existente.
"""
...
CONFIRM = "ACTIVATE-DESKTOP-RUNTIME-RUNNER"
...
def default_runner_home() -> Path:
    ...
    return Path(local).joinpath(*DEDICATED_RUNNER_DIR)


def discover_runner(explicit: Path | None = None) -> Path | None:
    expected = default_runner_home().resolve()
    if explicit is not None and explicit.resolve() != expected:
        raise ActivationError(
            "runner_home_not_authorized",
            "runner_home deve ser o diretório dedicado do desktop runtime",
        )
    try:
        return expected if runner_contract(expected) else None
    except OSError:
        return None
...
def _load_watchdog_runtime():
    ...
    spec = importlib.util.spec_from_file_location("desktop_pc24x7_runtime_watchdog_helpers", WATCHDOG)
...
def _runner_log_path(root: Path) -> Path:
    log_root = Path(os.environ.get("LOCALAPPDATA") or str(root)) / "DesktopPC24x7" / "GitHubRunner" / "logs"
